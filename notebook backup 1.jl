...
# ╔═╡ c9d443f4-dec9-4fc8-b5aa-042b1869a485
using CairoMakie

# ╔═╡ 03546a57-13fe-4d1f-90ef-cd952272beca
using Optimization, OptimizationOptimJL

# ╔═╡ f6996044-f3f9-45d8-9638-471c04fb3a85
using LinearAlgebra

# ╔═╡ 39986dcd-8f25-4885-b021-4da0964ed5ea
using ADTypes, Enzyme, Zygote, FiniteDiff, Ipopt

# ╔═╡ cb5671d3-383e-4ac9-8f98-aac121395a56
# ╠═╡ disabled = true
#=╠═╡
using OptimizationBase, OptimizationIpopt
  ╠═╡ =#

# ╔═╡ de3eab54-6d68-4833-a78e-877df07b8112
function dyn(x, u, r)
	_, _, ϕ = x
	return [u * cos(ϕ), u * sin(ϕ), r]
end

# ╔═╡ f6b40ab6-ba1b-44d1-9ab5-d34b8ea90c3e
function dynamics_cons(x, u, r, T, N)
	δt = T / N
...
# ╔═╡ d6606352-6a45-406f-a943-cc22fb7ee458
# ╠═╡ disabled = true
#=╠═╡
begin
	dynamics_cons([(1., 2., 3.), (4., 5., 6.), (7., 8., 9.)], [1., 1., 1.], [0.2, 0.2, 0.2], 4., 3)
end
  ╠═╡ =#

# ╔═╡ efe0f0d6-923e-4744-8994-cc88bae43ee7
function rotate(p, ϕ)
	return (cos(ϕ) * p[1] + sin(ϕ) * p[2], -sin(ϕ) * p[1] + cos(ϕ) * p[2])
end

# ╔═╡ 3d0a680d-1622-4537-a696-cf001b1b1648
"""
...
# ╔═╡ d51e9fa4-67e7-4600-8fdc-2f9ae8cc56cb
function corner_cons(ϕ, x, y, l1, l2, rw, rh, i)
	c = corner(ϕ, rw, rh, i) .+ [x, y]
	return 1 - min(abs(c[1]) / (l1/2), abs(c[2]) / (l2/2))
end

# ╔═╡ e0ca4733-6617-41cc-8b27-c849e2e7965a
function robot_corner_cons(ϕ, x, y, l1, l2, rw, rh, i)
	c = corner(ϕ, l1, l2, i)

	# calculate the coordinates of the corner c in the base of the robot
	p = rotate(c .- (x, y), -ϕ)
	
	return max(abs(p[1]) / (rw/2), abs(p[2]) / (rh/2)) - 1
end

# ╔═╡ 16b19481-bdc5-4d77-84d7-f3b6de93eb5d
function single_geom_cons(x, dimensions)
	x_, y_, ϕ_ = x
	rw, ry, l1, l2 = dimensions
	
	return [[corner_cons(ϕ_, x_, y_, l1, l2, rw, ry, i) for i in 1:4]; [robot_corner_cons(ϕ_, x_, y_, l1, l2, rw, ry, t) for t in 1:4]]
end

# ╔═╡ 37139490-b869-4ecb-97f0-026e66b2b2b4
# ╠═╡ disabled = true
#=╠═╡
# test single_geom_cons
single_geom_cons((1., 2., 3.), (1., 2., 3., 4.))
  ╠═╡ =#

# ╔═╡ deb8a86f-4297-4057-adb9-e648cc34d13a
function geom_cons(x, dimensions)
	cons = (s -> single_geom_cons(s, dimensions)).(x)
	return reduce(vcat, cons)#collect(Iterators.flatten(cons))
end

# ╔═╡ ae70e5e1-e516-40ba-8658-68cb2a24e600
# ╠═╡ disabled = true
#=╠═╡
#test geom_cons
geom_cons([[1., 2., 3.], [4., 5., 6.]], (1., 2., 3., 4.))
  ╠═╡ =#

# ╔═╡ 85f10241-f9b1-48fd-accc-d1f7630f4906
function problem_cons(res, x, p)
	# unpack the parameters
	N, dimensions, U_max, x_0, x_f = p
	...
	end_cons = [x_[begin] .- x_0;]#; x_[end] .- x_f]
	
	#res .= [dynamics_cons(x_, u_, r, T, N); end_cons; geom_cons(x_, dimensions); speed_cons]
	res .= [dynamics_cons(x_, u_, r, T, N); end_cons; speed_cons]
end

# ╔═╡ a9053b21-6e99-4998-86aa-c0e9b5e2d51a
function lerp(a, b, x)
	return b .* x .+ (1-x) .* a
end

# ╔═╡ dd247cf0-66ac-4e55-95a1-1ba1632b7913
...
# ╔═╡ baf919ae-6626-43dd-affb-84f8651b9311
# ╠═╡ disabled = true
#=╠═╡
function initialConditionsRect(N, U_max, x_0, x_f)
	p0 = [x_0...][1:2]
	pf = [x_f...][1:2]
	ϕ = atan(pf[2] - p0[2], pf[1] - p0[1])
...
# ╔═╡ a4bb7f42-d692-4b58-82d3-96bc02280106
function objective(u, p)
	N = p[begin]
	return u[end] + 30 * norm(u[3*N-2 : 3*N] .- p[5])
end

# ╔═╡ 6f5ece63-c14e-48b8-948d-152be60e155f
function solve_problem(N::Int, U_max::Tuple{Float64, Float64}, x_0::NTuple{3, Float64}, x_f::NTuple{3, Float64}, dimensions::NTuple{4, Float64})
	# paramètres : N, U_max, x_0, x_f, dimensions géométriques
	params = (N, dimensions, U_max, x_0, x_f)
	
	# initialisation de x0, en calculant un T et des positions initiales raisonnables
	x, u, r, T = initialConditionsStraight(N, U_max, x_0, x_f)
	x0 = [reduce(vcat, x); u; r; T]
	#return (s -> Tuple(s)).(eachcol(reshape(x0[1 : 3*N], (3, N)))), x0[end]
	
	# initialisation des contraintes
	lcons = [ones((N-1) * 3 + 3); zeros(2*N)]
	ucons = [ones((N-1) * 3 + 3); fill(Inf,2*N)]

	#ADTypes.AutoZygote()
	optim_f = OptimizationFunction(objective, ADTypes.AutoFiniteDiff(), cons = problem_cons)
	prob = Optimization.OptimizationProblem(optim_f, x0, params, lcons=lcons, ucons=ucons)
	sol = solve(prob, IPNewton(), maxtime = 240)

	#return the calculated positions as a vector of tuples, and the total time
	return (s -> Tuple(s)).(eachcol(reshape(sol.u[1 : 3*N], (3, N)))), sol.u[end]
end

# ╔═╡ cb21540e-fb1a-426d-a93e-18d4dc51fadc
begin
	sol = solve_problem(10, (2.4, 1.5), (0., -2., π/2), (2., 0., 0.), (1.128, 0.720, 2.4, 0.9))
end

# ╔═╡ ac49f2f0-b2bb-4aa1-8581-a6b73c560951
...
# ╔═╡ 1f397aa1-e4ea-4539-9eb3-b25ff28b13da
function plot_positions!(ax, pos, size)
    for p in pos
		plot_robot!(ax, p, size)
	end
end

# ╔═╡ 4d9fe9c8-d6a1-4b07-a7aa-be3899bc7b0a
let f=Figure(), ax = Axis(f[1, 1], aspect = DataAspect())
	pos, T = sol
	
	plot_terrain!(ax, 2.4, 0.9)
	plot_positions!(ax, pos, (1.128, 0.720))
...
manifest_format = "2.0"
project_hash = "445feb4edc2ea728a2c0854b50a896e20c2e758b"
